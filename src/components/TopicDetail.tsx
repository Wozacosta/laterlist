...
const UnreadItemList = memo(function UnreadItemList({
  ...
  const handlePushToCalendar = useCallback(async (item: Item) => {
    try {
      ...
      if (data.ok) {
        alert(`Added "${item.title}" to ${data.provider === "google" ? "Google" : "Proton"} Calendar`);
      } else {
        ...
        {items.map((item) => (
          <div key={item.id}>
            <div className="flex items-center gap-2 rounded border border-gray-200 bg-white px-3 py-2 dark:border-gray-800 dark:bg-gray-950">
              ...
              <button
                type="button"
                onClick={() => handlePushToCalendar(item)}
                className="shrink-0 text-gray-300 hover:text-blue-500 dark:text-gray-700 dark:hover:text-blue-400 transition-colors"
